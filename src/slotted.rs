...
impl<B: ByteSlice> Slotted<B> {
    ...
    pub fn capacity(&self) -> usize {self.body.len()}

    pub fn num_slots(&self) -> usize {self.header.num_slots as usize}

    pub fn free_space(&self) -> usize {
        self.header.free_space_offset as usize - self.pointer_size()
    }

    fn pointer_size(&self) -> usize { size_of::<Pointer>() * self.num_slots()}

    fn pointers(&self) -> Pointers<&[u8]> {
        Pointers::new_slice(&self.body[..self.pointer_size()]).unwrap()
    }

    fn data(&self, pointer: Pointer) -> &[u8]{&self.body[pointer.range()]}
}
...
impl<B: ByteSliceMut> Slotted<B> {
    ...
    pub fn resize(&mut self, index: usize, len_new: usize) -> Option<()> {
        ...
        self.body.as_bytes_mut().copy_within(shift_range, free_space_offset_new);
        let mut pointers_mut = self.pointers_mut();
...
impl<B: ByteSlice> Index<usize> for Slotted<B> {
    ...
    fn index(&self, index: usize) -> &Self::Output {self.data(self.pointers()[index])}
}
