use zerocopy::{FromBytes, AsBytes, ByteSlice, ByteSliceMut, LayoutVerified};
...
impl<B: ByteSliceMut> Node<B> {
    pub fn initialize_as_leaf(&mut self) { self.header.node_type = NODE_TYPE_LEAF; }
    pub fn initialize_as_branch(&mut self) {self.header.node_type = NODE_TYPE_BRANCH; }
}
